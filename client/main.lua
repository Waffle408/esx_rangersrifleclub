...
RegisterNetEvent("rid_rangersrifle:sendShop")
AddEventHandler("rid_rangersrifle:sendShop", function(shopItems)
...
function OpenShopMenu(zone)
    ...
    TriggerServerEvent("rid_rangersrifle:stockUpdate")
    ...
    ESX.UI.Menu.Open("default", GetCurrentResourceName(), "shop", {
        ...
        ESX.TriggerServerCallback("rid_rangersrifle:buyWeapon", function(bought)
            if bought then
...
AddEventHandler("rid_rangersrifle:hasEnteredMarker", function(zone)
...
AddEventHandler("rid_rangersrifle:hasExitedMarker", function(zone)
    CurrentAction = nil
    ESX.UI.Menu.CloseAll()
end)
        ...
        for k, v in pairs(Config.Zones) do
            for i = 1, #v.Locations, 1 do
                ...
                if distance < 50.0 then
                    wait = 1
                    if PlayerData.job and PlayerData.job.name == "ranger" then
    ...
    while true do
        ...
        if PlayerData.job and PlayerData.job.name == "ranger" then
            if isInMarker and not HasAlreadyEnteredMarker then
                ...
                TriggerEvent("rid_rangersrifle:hasEnteredMarker", currentZone)
            end
        end
        ...
        if not isInMarker and HasAlreadyEnteredMarker then
            ...
            TriggerEvent("rid_rangersrifle:hasExitedMarker", LastZone)
        end
        Citizen.Wait(wait)
    end
end)
