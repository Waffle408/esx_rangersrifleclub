...
ESX.RegisterServerCallback('rid_rangersrifle:getShop', function(source, cb)
	cb(shopItems)
end)

ESX.RegisterServerCallback('rid_rangersrifle:buyWeapon', function(source, cb, weaponName, zone)
	...
	if price == 0 then
		print(('rid_rangersrifle: %s attempted to buy a unknown weapon!'):format(xPlayer.identifier))
		cb(false)
	else
		if xPlayer.hasWeapon(weaponName) then
			...
			if stock == 0 or stock < 0 then
				print(('rid_rangersrifle: %s attempted to buy 0 stock weapon!'):format(xPlayer.identifier))
				xPlayer.showNotification(_U('no_stock'))
				cb(false)
			else
				if zone == 'rangersrifle' then
					local money_type = ''
...
RegisterServerEvent('rid_rangersrifle:stockUpdate')
AddEventHandler('rid_rangersrifle:stockUpdate', function()

	-- Clear Current Table Inserts To prevent table stacking
	shopItems["rangersrifle"] = {}

	MySQL.Async.fetchAll('SELECT * FROM rangersrifle', {}, function(result)
		for i=1, #result, 1 do
		...
		TriggerClientEvent('rid_rangersrifle:sendShop', -1, shopItems)
	end)
end)

Citizen.CreateThread(function()
    print("rid_rangersrifle: Started!")
...
function weaponRandomizer(d,h,m)
	...
	print("rid_rangersrifle: New Weapons in Black Market")
...
function weaponRestock()
	-- On Resource Restart
	print("rid_rangersrifle: Refilling all weapon stock")
